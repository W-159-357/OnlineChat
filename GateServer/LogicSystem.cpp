...
LogicSystem::LogicSystem()
{
	...
	RegPost("/get_varifycode", [](std::shared_ptr<HttpConnection> connection) {
		...
		Json::Reader reader;
		Json::Value src_root;
		bool parse_success = reader.parse(body_str, src_root);	// 解析body_str后放入src_root
		if (!parse_success) {
		...
		GetVarifyRsp rsp = VerifyGrpcClient::GetInstance()->GetVerifyCode(email);		// 获取验证码
	...
	RegPost("/user_register", [](std::shared_ptr<HttpConnection> connection) {
		...
		if (varify_code != src_root["varifycode"].asString()) {		// 判断验证码是否相同
